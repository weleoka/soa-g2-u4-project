...
We will also implement a message que using (Kafka/RabbitMQ? not decided...).
...
You can find API documentation generated by Swagger at for example: `http://localhost:8085/swagger-ui/index.html` and then `/v3/api-docs` in path input field. Or simply go to URL `http://localhost:8082/v3/api-docs` for the raw version.
